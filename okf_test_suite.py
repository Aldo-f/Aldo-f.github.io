...
rc = subprocess.run(
    ["curl", "-fsS", "-o", "/dev/null", "-w", "%{http_code}", SITE_URL],
    capture_output=True,
    text=True,
).stdout.strip()
if rc != "200":
...
res = subprocess.run(
    [str(script), "http://127.0.0.1:8096/health"], capture_output=True, text=True
)
...
res = subprocess.run(
    ["python3", str(wrapper)], input=payload, capture_output=True, text=True
)
if res.returncode != 0:
