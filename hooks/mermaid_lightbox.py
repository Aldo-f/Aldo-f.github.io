...
def on_page_content(html, page, config, files):
    ...
    # Add mermaid script and styles
    if MERMAID_JS_URL not in html:
        # Add JS before closing body
        html = html.replace('</body>', 
            f'<script src="{MERMAID_JS_URL}"></script>\n'
            f'<script>{_LIGHTBOX_ADDON_JS}</script>\n'
            f'<link rel="stylesheet" href="assets/css/mermaid-lightbox.css">\n'
            '</body>'
        )
...
def on_config(config, **kwargs):
    """Ensure mermaid CSS is included."""
...
def on_post_build(config):
    ...
    print(f"mermaid-lightbox: wrote {css_path}")
