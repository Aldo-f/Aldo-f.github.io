...
def _inject_key(js_template: str, api_key: Optional[str]) -> str:
...
def on_config(config, **kwargs):
    ...
    This hook registers the chat.js and chat.css scripts that power the RAG FAB.
    The files are emitted by on_post_build after reading the API key from the .env file.
    """
    # Get the real API key from the OKF RAG .env file
    okf_env_path = Path("/home/aldo/dev/02-ai-okf-home-lab/.env")
    rag_api_key = None
    if okf_env_path.exists():
        for line in okf_env_path.read_text().splitlines():
            if line.startswith("RAG_API_KEY="):
                rag_api_key = line.split("=", 1)[1]
                break

    # If the key is missing, use placeholder so the widget will be hidden at runtime
    if not rag_api_key:
        rag_api_key = "[MISSING_RAG_API_KEY]"

    # Update chat.js with the real key
    js_content = _CHAT_JS_TEMPLATE.replace("{{RAG_API_KEY}}", rag_api_key)
    config.extra_javascript.append(JS_NAME)
    config.extra_css.append(CSS_NAME)

    # Write the JavaScript and CSS files to the workspace
    (Path(JS_NAME)).parent.mkdir(parents=True, exist_ok=True)
    (Path(JS_NAME)).write_text(js_content)

    (Path(CSS_NAME)).parent.mkdir(parents=True, exist_ok=True)
    (Path(CSS_NAME)).write_text(_CHAT_CSS)
...
def on_post_build(*, config, **kwargs):
    ...
    js_content = _inject_key(_CHAT_JS_TEMPLATE, api_key)
    css_content = _CHAT_CSS

    site_dir = Path(config.get("site_dir", "site"))
    js_out = site_dir / JS_NAME
    css_out = site_dir / CSS_NAME
    js_out.parent.mkdir(parents=True, exist_ok=True)
    css_out.parent.mkdir(parents=True, exist_ok=True)
    js_out.write_text(js_content)
    css_out.write_text(css_content)
    print(f"chat: emitted {js_out} and {css_out}")
