...
REPO_DEST_MAP = {
    ...
    "06-apps-passive-income": [
        f"{DOCS_SITE_DOCS}/passive-income",
    ...
    "06-apps-neo-brutalist-home": [
        f"{DOCS_SITE_DOCS}/neo-brutalist-home",
...
class DocWatcher:
    ...
    def scan_all_repositories(self):
        ...
        for item in APPS_DIR.iterdir():
            if (
                item.is_dir()
                and item.name.startswith("06-apps-")
    ...
    def run_once(self):
        ...
        if changes:
            ...
            if integrated:
                ...
                post_sync = (
                    Path(__file__).resolve().parent.parent / "scripts" / "post_sync.sh"
                )
                if post_sync.exists():
                    try:
    ...
    def run_daemon(self):
        ...
        print(
            f"Starting documentation watcher daemon (checking every {CHECK_INTERVAL}s)"
        )
        try:
            while True:
                self.run_once()
