...
def main() -> int:
    ...
    check(
        "parity scrum-week",
        f("Start van de Scrum-week", "-") == "start-van-de-scrum-week",
    )
    ...
    check(
        "en->nl entry",
        m.get("/blog/2026/01/01/welcome-to-the-blog/")
        == "/nl/blog/2026/01/01/welkom-op-de-blog/",
        str(m),
    )
    check(
        "reverse entry",
        m.get("/nl/blog/2026/01/01/welkom-op-de-blog/")
        == "/blog/2026/01/01/welcome-to-the-blog/",
    )
    ...
    check(
        "pair mapped",
        m.get("/blog/2026/01/01/pair/") == "/nl/blog/2026/01/01/paar/",
        str(m),
    )
    check(
        "unmirrored excluded",
        "/blog/2026/01/01/only-en/" not in m
        and "/nl/blog/2026/01/01/solo-nl/" not in m,
    )
    ...
    check(
        "json written + shape",
        isinstance(data, dict) and "/blog/2026/01/01/pair/" in data,
    )
